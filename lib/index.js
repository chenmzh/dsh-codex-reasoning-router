...
function apply(ctx, config) {
	...
	ctx.on("agent/request", async (payload, next) => {
		...
		if (installed.get(payload.agent) === void 0) return request;
	...
	ctx.on("llm/stream", (options, next) => {
		...
		if (isAgentLoopRequest(options) && options.sessionId !== void 0) {
			if ([...installed.keys()].find((agent) => agent.id === options.sessionId) !== void 0 && (options.provider !== config.lunaProvider || options.model !== config.lunaModel)) throw new Error("dsh-codex-reasoning-router: root LLM stream must remain " + config.lunaProvider + "/" + config.lunaModel + "; observed " + options.provider + "/" + options.model + ". Dispatch was blocked without rewriting the request.");
		}
		return next();
	});
	for (const agent of ctx.agents.roots()) attach(agent);
	ctx.effect(() => validateModels(ctx, config).then(() => () => void 0), `${PACKAGE_NAME}: validate configured model catalog`);
	ctx.effect(() => () => {
